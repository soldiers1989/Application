 ...
 * 专题实体类
 */
public class ThemesInfo implements Parcelable {
    /**
     * limit : 1000
     * subscribed : []
    ...
    protected ThemesInfo(Parcel parcel) {
        limit = parcel.readInt();
        subscribed = parcel.createStringArrayList();
        others = new ArrayList<>();
        parcel.readList(others, Others.class.getClassLoader());
    }
    ...
    public void writeToParcel(Parcel dest, int flags) {
        dest.writeInt(limit);
        dest.writeStringList(subscribed);
        dest.writeList(others);
    }

    public static final Creator<ThemesInfo> CREATOR = new Creator<ThemesInfo>() {

        @Override
        public ThemesInfo createFromParcel(Parcel source) {
            return new ThemesInfo(source);
        }

        @Override
        public ThemesInfo[] newArray(int size) {
            return new ThemesInfo[size];
        }
    };
