...
public class BookContentAdapter extends BaseRecyclerViewAdapter<BookContentInfo.Data> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        itemViewHolder.btnBuy.setOnClickListener(view -> {
            String online = bookData.getOnline();
            if (TextUtils.isEmpty(online)) {
                Toast.makeText(mContext, R.string.no_books_available_for_sale, Toast.LENGTH_SHORT).show();
            } else {
                String url = online.substring(online.indexOf(":") + 1, online.indexOf(" "));
                ActivityHelper.startBooksDetailActivity(mContext, url);
            }
        });
